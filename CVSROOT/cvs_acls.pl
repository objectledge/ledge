...
#
#	DEFAULT		/usr/local/bin/cvs_acls.pl $USER
#
...
# PROGRAM LOGIC:
#
#	CVS passes us, through @ARGV, the username of the client, an
#	absolute directory pathname (the repository appended to your
#	$CVSROOT variable), and a list of filenames within that directory.
#
#	We walk through the avail file looking for a line that matches both
#	the username and repository.  We check both the client username from
#	the command line ($ARGV[0]) and the server username ($ENV{"USER"}).
#
...
#		- Otherwise *all* file arguments ($ARGV[2..$#ARGV]) must be
...
my $debug = $sc->param('Debug');
my $readonly = $sc->param('ReadOnly');

$cvsroot = $ENV{'CVSROOT'};
$availfile = $cvsroot . "/CVSROOT/avail";
$client_user = shift(@ARGV);
$server_user = $ENV{"USER"} if !($server_user = $ENV{"LOGNAME"});
...
if ($readonly) {
    print "**** Access denied: repository is READ-ONLY.\n";
    exit 1;
}
...
while (<AVAIL>) {
    ...
    # user considered "in user list" if actually in list or is NULL
    $in_user = (!$u
		|| grep ($_ eq $client_user, split(/[\s,]+/,$u))
		|| grep ($_ eq $server_user, split(/[\s,]+/,$u)));
    print "$$ \user($client_user/$server_user) in user list: $_\n"
	if $debug && $in_user;
